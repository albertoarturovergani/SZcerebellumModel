    ...
    try:
        ...
        for i in range(len(pop_names)):
            for j in range(i + 1, len(pop_names)):
                ...
                row[f"cross_corr_{a}_{b}"] = float(np.max(np.abs(np.correlate((a_mean - np.mean(a_mean))/np.std(a_mean), (b_mean - np.mean(b_mean))/np.std(b_mean), mode='full')) / len(a_mean)))
                dt = 1.0
                max_lag_ms = 200
                max_lag_steps = int(max_lag_ms / dt)
                x, y = (a_mean - np.mean(a_mean)) / np.std(a_mean), (b_mean - np.mean(b_mean)) / np.std(b_mean)
                ...
                row[f"optimal_lag_ms_{a}_{b}"] = float(lags[window][best_idx])
                row[f"plv_{a}_{b}"] = float(np.abs(np.mean(np.exp(1j * (np.angle(hilbert(a_mean)) - np.angle(hilbert(b_mean)))))))
                row[f"te_{a}_to_{b}"] = 0.0
                row[f"te_{b}_to_{a}"] = 0.0
