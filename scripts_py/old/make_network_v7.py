...
def k_reserve_simple_v2(k, k0=0.75, sharpness=1e-3):
    k = np.array(k, dtype=float)  # garantisce compatibilità con scalari
    raw = 1 / (1 + np.exp(-sharpness * (k - k0)))
    eps = 1e-9
    return 0.5 + (raw - raw.min()) / (raw.max() - raw.min() + eps) * 0.5
...
def mirrored_sigmoid(k, midpoint, sharpness=20):
    # Funzione che mappa k ∈ [0.5, 1.0] in f ∈ [0.5, 1.0]
    # Centrata in midpoint (es. 0.6 per declino precoce, 0.9 per declino tardivo)
    raw = 1 / (1 + np.exp(-sharpness * (k - midpoint)))
    # Riscalata in [0.5, 1.0] con f(k=1.0)=1.0, f(k=0.5)=0.5, indipendentemente da sharpness
...
def filter_connections(hypothesis):
    ...
    'io_to_mli_io_to_stellate_cell': ('glutamate', 'io'),
    'io_to_mli_io_to_basket_cell': ('glutamate', 'io'),
...
def writeYAML_cerebellum_cx(k, 
    ...
    k_modulated = k_reserve_simple_v3(k, k0=reserve['k0'], sharpness=reserve['sharpness'])
    ...
    if 'cell_types' in config_data:
        for cell, props in config_data['cell_types'].items():
            ...
            #if 'count_ratio' in spatial: it should be only for mf but better take nautrally proportioned with glom
            #            spatial['count_ratio'] = np.round(float(spatial['count_ratio'] * k_modulated), 6)
    ...
    scale_weights(A, k, listConns, effect='downregulation') # connessioni scalate linearmente - agente patologizzante
    ...
    if do_dcnio:
        ...
        if os.path.exists(dcn_yaml):
            ...
            # 5. RIDUZIONE NEURONI
            if 'cell_types' in config_dcn:
                for cell, props in config_dcn['cell_types'].items():
                    spatial = props.get('spatial', {})
                    if 'density' in spatial:
                        spatial['density'] = np.round(float(spatial['density'] * k_modulated), 6)
                    if 'planar_density' in spatial:
                        spatial['planar_density'] = np.round(float(spatial['planar_density'] * k_modulated), 6)
                    #if 'count_ratio' in spatial:
                    #    spatial['count_ratio'] = np.round(float(spatial['count_ratio'] * k_modulated), 6)

            # **3 per bioplausibilità decadimento
            # il drive di dcn in no resilience è comunque simile nel caso k piuttosto che k_modulated 
            # il k_modulated di pc su dcn è per dare un "premio" alle cellule più grandi e quindi anche più resistenti a livello di connettività
            # e per renderle più simili ad altri decadimenti a mezza campana (hr) come in glom->, golgi->glom, mf-glom (bilancia input strutturale infatti)
            # mf con k_modulated su dcn per avere stesso andamento in HR nel caso di mf -> glom ed mf -> dcn
            if 'connectivity' in config_dcn:
                value = config_dcn['connectivity']['mossy_fibers_to_dcn_p']['indegree']
                config_dcn['connectivity']['mossy_fibers_to_dcn_p']['indegree'] = max(1, int(np.round(float(value * k_modulated**3), 6)))
                value = config_dcn['connectivity']['purkinje_to_dcn_i']['outdegree']
                config_dcn['connectivity']['purkinje_to_dcn_i']['outdegree'] = max(1, int(np.round(float(value * k_modulated**3), 6)))
                value = config_dcn['connectivity']['purkinje_to_dcn_p']['outdegree']
                config_dcn['connectivity']['purkinje_to_dcn_p']['outdegree'] = max(1, int(np.round(float(value * k_modulated**3), 6)))

            if 'partitions' in config_dcn:
                for layer in config_dcn['partitions']:
                    for key, value in config_dcn['partitions'][layer].items():
                        if isinstance(value, (int, float)):
                            config_dcn['partitions'][layer][key] = np.round(float(value * k_modulated), 6)
        ...
        if os.path.exists(dcn_io_yaml):
            ...
            # 5. RIDUZIONE NEURONI
            if 'cell_types' in config_io:
                for cell, props in config_io['cell_types'].items():
                    spatial = props.get('spatial', {})
                    if 'density' in spatial:
                        spatial['density'] = np.round(float(spatial['density'] * k_modulated), 6)
                    if 'planar_density' in spatial:
                    ...
                    #if 'count_ratio' in spatial:
                    #    spatial['count_ratio'] = np.round(float(spatial['count_ratio'] * k_modulated), 6)
            if 'partitions' in config_io:
                for layer in config_io['partitions']:
                    for key, value in config_io['partitions'][layer].items():
                        if isinstance(value, (int, float)):
                            config_io['partitions'][layer][key] = np.round(float(value * k_modulated), 6)
                            
            # conns modified because no morphology as cx cell types, while cell types has not modified conns since reduced have less conns as consequence
            # **3 per bioplausibilità decadimento
            # ** k_modulated per somiglianza con altre cell
            if 'connectivity' in config_io:
                value = config_io['connectivity']['dcn_i_to_io']['indegree']
                config_io['connectivity']['dcn_i_to_io']['indegree'] = max(1, int(np.round(float(value * k_modulated**3), 6)))
                value = config_io['connectivity']['io_to_dcn_p']['outdegree']
                config_io['connectivity']['io_to_dcn_p']['outdegree'] = max(1, int(np.round(float(value * k_modulated**3), 6)))
                value = config_io['connectivity']['io_to_dcn_i']['outdegree']
                config_io['connectivity']['io_to_dcn_i']['outdegree'] = max(1, int(np.round(float(value * k_modulated**3), 6)))
        ...
        scale_weights(A_dcn, k, listConns, effect='downregulation')
        ...
        scale_weights(A_dcn_io, k, listConns, effect='downregulation') # connessioni scalate linearmente - agente patologizzante
...
def main():
    ...
    try:
        reserve_dict = ast.literal_eval(args.reserve)
        assert isinstance(reserve_dict, dict)
    except Exception as e:
        print(f"❌ Invalid --reserve format: {e}")
        sys.exit(1)
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        writeYAML_cerebellum_cx(
            ...
            do_basal=True,
            do_dcnio=True,
            reserve={'k0': 0.75, 'sharpness': 1e-3},
            compensation={'state': 1.0, 'trasmitter': ['glutamate','gaba'], 'neurons': 'all'},
            sz_hyphothesis={'trasmitter': 'glutamate', 'neurons': 'all'}
        )
    else:
