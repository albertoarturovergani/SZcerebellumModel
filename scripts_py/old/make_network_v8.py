...
def writeYAML_cerebellum_cx(k, 
    ...
    if stim_name=='basal_vitro':
        weight_file_in = os.path.join(sim_path, "basal_vitro.yaml")
        weight_config_data = safe_load_yaml(weight_file_in)
        A = weight_config_data['simulations'][stim_name]['connection_models']
    else:
        weight_file_in = os.path.join(sim_path, f"{stim_name}_{k_str}.yaml")
        weight_config_data = safe_load_yaml(weight_file_in)
        A = weight_config_data['simulations'][stim_name]['connection_models']
    ...
    weight_config_data['simulations'][stim_name]['connection_models'] = A
    ...
    weights_outfile = os.path.join(sim_path, f"{stim_name}_{k_str}.yaml")
    ...
    sim_outfile = os.path.join(sim_path, f"{stim_name}_{k_str}.yaml")
    ...
    if do_dcnio:
        ...
        A_dcn = config_data_dcn['simulations'][stim_name]['connection_models']
        scale_weights(A_dcn, k, listConns, effect='downregulation')
        config_data_dcn['simulations'][stim_name]['connection_models'] = A_dcn
        if os.path.exists(dcn_vitro_yaml):
            ...
            config_data_dcn['simulations'][stim_name]['$import']['ref'] = f"../nest/{stim_name}_{k_str}.yaml#/simulations/{stim_name}/"
        ...
        A_dcn_io = config_data_dcn_io['simulations'][stim_name]['connection_models']
        scale_weights(A_dcn_io, k_modulated, listConns, effect='downregulation') # connessioni scalate linearmente - agente patologizzante
        config_data_dcn_io['simulations'][stim_name]['connection_models'] = A_dcn_io
        if os.path.exists(dcn_io_vitro_yaml):
            if '$import' in config_data_dcn_io:
                config_data_dcn_io['$import']['ref'] = f"./dcn_io_{k_str}.yaml#/"
            config_data_dcn_io['simulations'][stim_name]['$import']['ref']=f"./dcn_vitro_nest_{k_str}.yaml#/simulations/{stim_name}/"
...
def main():
    ...
    parser.add_argument('--stim_name', type=str, default='basal_vitro', help='Stimulus configuration name (e.g., basal_vitro, pawanStim, etc.)')
    parser.add_argument('--sim_name', type=str, default='basal_activity', help='Stimulus configuration name (e.g., basal_activity, pawanStim, etc.)')
