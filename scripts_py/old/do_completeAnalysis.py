...
def plot_time_series(df_all, pop="mossy", dt=0.1, alpha=0.15, linewidth=1.0, dpi=300):
    import numpy as np
    import matplotlib.pyplot as plt
...
def loadData(data_path: Path, mainEventKey: str, condition_name: str, use_top_quartile: bool = False, THR=50,
             selection_mode: str = "percentile", selection_percentile: float = 0.99, method: str = "mean",
             downsample_target_rate_hz: float = None) -> pd.DataFrame:
    dt = 0.1
    ...
    for path in data_path.rglob("*_deg_*/3.function"):
        ...
        stim_profile = []
        yaml_file = path.parent / "2.stimulus" / f"basal_vitro_{k:.3f}.yaml"
        if yaml_file.exists():
            import yaml
        ...
        row = {
            ...
            "te_mossy_to_purk": compute_transfer_entropy_binned(m_mean, p_mean),
            "te_purk_to_mossy": compute_transfer_entropy_binned(p_mean, m_mean),
    ...
    plot_time_series(df_summary, pop="mossy")
    plot_time_series(df_summary, pop="purkinje")
    plot_tf(df_summary)
...
def main():
    ...
    df_summary, spike_dfs = loadData(base_dir, mainEventKey="burst", condition_name="saccades", use_top_quartile=True, selection_mode="percentile", method='mean')
