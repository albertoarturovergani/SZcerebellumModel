...
def bootstrap_te_lag_plv(mossy_fibers_trim, purk_trim, dt=0.1, n_boot=50, frac=0.7):
    """
    Stima la variabilità (std) di lag, TE e PLV rispetto alla popolazione
    usando bootstrap sulle unità.

    mossy_fibers_trim, purk_trim: array 2D (neuroni x tempo) dopo trimming
    dt: passo temporale in ms
    n_boot: numero di campioni bootstrap
    frac: frazione di neuroni da campionare ogni volta
    """
    n_m = mossy_fibers_trim.shape[0]
    n_p = purk_trim.shape[0]
    if n_m < 2 or n_p < 2:
        return {
            "lag_std": np.nan,
            "te_m2p_std": np.nan,
    ...
    for _ in range(n_boot):
        idx_m = np.random.choice(n_m, max(2, int(np.ceil(frac * n_m))), replace=True)
        idx_p = np.random.choice(n_p, max(2, int(np.ceil(frac * n_p))), replace=True)

        m_boot = mossy_fibers_trim[idx_m, :].mean(axis=0)
        p_boot = purk_trim[idx_p, :].mean(axis=0)

        # lag
        _, lag_b = compute_cross_corr_and_lag(m_boot, p_boot, dt)
        lag_vals.append(lag_b)

        # TE
        te_m2p_vals.append(compute_transfer_entropy_binned_old(m_boot, p_boot))
        te_p2m_vals.append(compute_transfer_entropy_binned_old(p_boot, m_boot))

        # PLV
        plv_vals.append(compute_plv(m_boot, p_boot))

    return {
        "lag_std": float(np.std(lag_vals, ddof=1)),
        "te_m2p_std": float(np.std(te_m2p_vals, ddof=1)),
        "te_p2m_std": float(np.std(te_p2m_vals, ddof=1)),
        "plv_std": float(np.std(plv_vals, ddof=1)),
    }


def fmt_sigma(s):
    s = float(s)
    return f"{int(s)}" if s.is_integer() else f"{s:.3g}"
...
def exp_decay(t, A, tau, C):
    x = -t / max(tau, 1e-9)
    x = np.clip(x, -709, 709)  # evita overflow/underflow di exp
    return A * np.exp(x) + C
...
def loadData(data_path: Path, mainEventKey: str, 
    ...
    for path in data_path.rglob("*_deg_*/3.function"):
        ...
        mossy_fibers_trim = mossy_fibers[:, start:end]
        purk_trim = purk[:, start:end]
        ...
        plot_tau_fits_mean_rise_decay(m_mean, burst_onsets_ms, dt=0.1,
                              title="mossy_fibers - Tau Fit Examples",
                              savepath=str(Path(base_dir) / f"tau_fit_mossy_fibers_k={k:.3f}.png"), color='green')
        ...
        plot_firing_rate_panels(
            ...
            atrophy=atrophy,
            stim_to_mossy_fibers_cosine=stim_to_mossy_fibers_cosine,
        ...
        # PSTH mossy_fibers
        average_psth(spike_dfs[k], pop="mossy_fibers", event_times=burst_onsets_ms,
                     plot_raster=True,
                     save_path=f"./{base_dir}/mossy_fibers_psth_{fmt_k(k)}.png",
        ...
        average_psth(spike_dfs[k], pop="purkinje", event_times=burst_onsets_ms,
                     plot_raster=True, 
                     save_path=f"./{base_dir}/purkinje_psth_{fmt_k(k)}.png",
                     condition_name=f"k={k:.3f}",
                     df_row_with_stim=row,
                     dt=0.1)
        # Raster mossy_fibers
        plot_classical_raster(spike_dfs[k], row, pop="mossy_fibers",
                              window=(-200, 200),
                              save_path=f"./{base_dir}/mossy_fibers_raster_classic_{fmt_k(k)}.png",
                              event_times=burst_onsets_ms,
                              title=f"Raster aligned to burst – k={k:.3f}")
        # Raster Purkinje
        plot_classical_raster(spike_dfs[k], row, pop="purkinje",
                              window=(-200, 200),
                              save_path=f"./{base_dir}/purkinje_raster_classic_{fmt_k(k)}.png",
                              event_times=burst_onsets_ms,
                              title=f"Raster aligned to burst – k={k:.3f}")
    ...
    plot_epoched_firing_rate(
        df=df_summary,
        trace_column="mossy_fibers_timeseries",
        pop_name="mossy_fibers Fibres",
        ...
        operator=np.max,
        dt=0.1,
        save_path=f"./{base_dir}/mossy_fibers_epoched.png"
    )
    plot_epoched_firing_rate(
        df=df_summary,
    ...
    plot_time_series(df_summary, pop="mossy_fibers", base_dir=base_dir)
    ...
    plot_cosine_similarity_vs_atrophy(df_summary, column_name="cosine_similarity_stim_mossy_fibers", save_path=f"./{base_dir}/cosine_stim_mossy_fibers.png")
    ...
    plot_cosine_similarity_vs_atrophy(df_summary, column_name="cosine_similarity_mossy_fibers_purk", save_path=f"./{base_dir}/cosine_similarity_mossy_fibers_purk.png")
    plot_stim_vs_mossy_fibers_rate(df_summary, save_path=f"./{base_dir}/stim_vs_mf_rate.png")
    ...
    #df_summary.to_pickle(Path(base_dir) / "B_collective_results" / "2.functions" / f"sigma_{sigma}" / f"{sigma}_df.pkl")
    # Salvataggio del DataFrame (robusto)
    sigma_fmt = fmt_sigma(sigma)
    out_dir = Path(base_dir) / "B_collective_results" / "2.functions" / f"sigma_{sigma_fmt}"
    out_dir.mkdir(parents=True, exist_ok=True)
    df_summary.to_pickle(out_dir / f"{sigma_fmt}_df.pkl")
...
def move_png_to_sigma_folder(path: Path, sigma: float):
    sigma_fmt = fmt_sigma(sigma)
    dest_dir = path / "B_collective_results" / "2.functions" / f"sigma_{sigma_fmt}"
    dest_dir.mkdir(parents=True, exist_ok=True)
    png_files = list(path.glob("*.png"))
    for f in png_files:
        shutil.move(str(f), dest_dir / f.name)
