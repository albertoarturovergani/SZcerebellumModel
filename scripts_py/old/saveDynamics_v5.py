...
def process_simulation(main_dir, file_name, rel, simulation_types):
    ...
    rel = f"{rel:.3f}"
    hdf5_file = os.path.join(main_dir, f"results_{rel}", f"{file_name}_{rel}.hdf5")
...
def main():
    ...
    file_name = 'mouse_cereb_dcn_io_nest'
    ...
    df_dynamics.to_pickle(os.path.join(main_dir, "dynamics_fr_compensation_linear.pkl"))
