...
def main():
    ...
    parser.add_argument("root", nargs="?", default=".", help="Main directory")
    ...
    for result_dir in root_path.glob('*_deg_*/3.function'):
        ...
        mossy_rates = [rate for pop, rate in zip(firing_data["pop"], firing_data["rate"]) if pop == "mossy"]
        purkinje_rates = [rate for pop, rate in zip(firing_data["pop"], firing_data["rate"]) if pop == "purkinje"]

        if not mossy_rates or not purkinje_rates:
        ...
        ax.plot(t, mossy_mean, label="Mossy fibers", linewidth=1.5)
        ...
        f_mossy, psd_mossy = compute_population_psd(mossy_array, fs=1000/dt)
        f_purk, psd_purk = compute_population_psd(purkinje_array, fs=1000/dt)

        fooof_out_mossy = analyze_and_plot_fooof(mossy_mean, 1000/dt, f"FOOOF – Mossy (k={kvalue:.3f})", output_dir_k / f"fooof_mossy_{kvalue:.3f}")
        ...
        summary = {
            ...
            "fooof": {"mossy": fooof_out_mossy, "purkinje": fooof_out_purkinje},
            #"psd": {"mossy": {"f": f_mossy, "psd": psd_mossy}, "purkinje": {"f": f_purk, "psd": psd_purk}},
        }
