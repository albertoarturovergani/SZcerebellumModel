...
def generate_stimulus_timeline(yaml_path, stimName='basal_activity'):
...
def process_reconstruction_yamls(base_dir: Path, stimName='basal_activity'):
    ...
    for yaml_path in yaml_paths:
        if generate_stimulus_timeline(yaml_path, stimName):
            count += 1
...
def plot_firing_rate_panels(m_mean, p_mean, mossy, purk, stim, dt, start, end, atrophy, stim_to_mossy_cosine, stim_to_purk_cosine, k, base_dir):
    ...
    # --- Mossy ---
    m_min = np.quantile(mossy[:, start:end], q_low, axis=0)[:len(m_mean)]
    m_max = np.quantile(mossy[:, start:end], q_high, axis=0)[:len(m_mean)]
    ...
    ax1.set_title(f"Mossy firing rate – Atrophy {atrophy:.1f}% – cos(stim,m)={stim_to_mossy_cosine:.2f}")
    ...
    # --- Mossy vs Purkinje ---
    ax3.plot(t, m_mean, label="Mossy", linestyle='-', linewidth=0.75, color="orange")
    ...
    ax3.set_title(f"Mossy vs Purkinje – cos(m,p)={cos_mp:.2f}")
...
def printSynTime(stimName='basal_activity'):
    import yaml
    from pathlib import Path
    ...
    conn_models = data["simulations"][stimName]["connection_models"]
...
def plot_tf(df_all, base_dir):
    ...
    for cond, group in df_all.groupby("condition"):
        # MF → PC
        if len(group) >= 2:
            r1, p1 = pearsonr(group["Atrophy Factor (%)"], group["te_mossy_to_purk"])
        ...
        # PC → MF
        if len(group) >= 2:
            r2, p2 = pearsonr(group["Atrophy Factor (%)"], group["te_purk_to_mossy"])
    ...
    try:
        stat_w, pval_w = wilcoxon(
            df_all["te_mossy_to_purk"],
            df_all["te_purk_to_mossy"]
        )
        print(stat_w)
        ax1.text(
            0.05, 0.95,
    ...
    df_all["ΔTE"] = df_all["te_mossy_to_purk"] - df_all["te_purk_to_mossy"]
    if len(df_all) >= 2:
...
def plot_firing_rate_vs_k(df_all, save_path=None):
    ...
    ax = plt.gca()
    palette = {"mossy": "#1f77b4", "purkinje": "#ff7f0e"}
    spearman_vals = {}
...
def plot_time_series(df_all, pop="mossy", base_dir=Union[str, Path], dt=0.1, alpha=0.15, linewidth=1.0, dpi=300):
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    # Colormap: arancio per mossy, blu per purkinje
    cmap = cm.Oranges.reversed() if pop == "mossy" else cm.Blues.reversed()
...
def plot_classical_raster(df_spikes, df_row_with_stim, pop="mossy", window=None,
...
def average_psth(df, pop="mossy", event_times=None, window=(-200, 200), bin_size=5.0,
...
def plot_raster(df_spikes, pop="mossy", time_window=None, max_units=100):
...
def plot_stim_vs_mossy_rate(df_all, stim_column="stim_profile", mossy_column="mossy_mean_rate", save_path=None):
    import matplotlib.pyplot as plt
    import seaborn as sns
    from scipy.stats import spearmanr
    import numpy as np
    ...
    # Correlazione Spearman
    rho, pval = spearmanr(df_all["stim_mean_rate"], df_all[mossy_column])
    ax.legend(title=f"ρ = {rho:.2f}, p = {pval:.2e}", loc="upper left")

    ax.set_title("Stimulus Mean Rate vs Mossy Firing Rate")
    ax.set_xlabel("Stimulus Mean Rate (Hz)")
    ax.set_ylabel("Mossy Mean Firing Rate (Hz)")
    ax.grid(True)

    plt.tight_layout()
    if save_path:
        plt.savefig(save_path, dpi=300)
        print(f"✅ Stimulus vs Mossy rate plot salvato in: {save_path}")
    plt.close()
...
def loadData(data_path: Path, mainEventKey: str, 
    ...
    for path in data_path.rglob("*_deg_*/3.function"):
        ...
        mossy = select_quartile(data, 'mossy', mode=selection_mode, percentile=selection_percentile) if use_top_quartile else np.vstack([
            r for i, r in enumerate(data["rate"]) if data["pop"][i] == "mossy"])
        ...
        if method == "mean":
            ...
            m_mean = np.median(mossy[:, start:end], axis=0)
            p_mean = np.median(purk[:, start:end], axis=0)
        elif method.startswith("quantile_"):
            q = float(method.split("_")[1])
            m_mean = np.quantile(mossy[:, start:end], q, axis=0)
            p_mean = np.quantile(purk[:, start:end], q, axis=0)
        else:
            raise ValueError("Metodo non valido per la stima temporale")

        m_sem, p_sem = mossy[:, start:end].std(0)/np.sqrt(mossy.shape[0]), purk[:, start:end].std(0)/np.sqrt(purk.shape[0])

        mossy_quantile_value = float(np.quantile(m_mean, selection_percentile))
        ...
        if stim_profile:
            ...
            stim_to_mossy_cosine = cosine_similarity(m_mean, stim)
            stim_to_purk_cosine = cosine_similarity(p_mean, stim)
        else:
            stim = None
            stim_to_mossy_cosine = np.nan
        ...
        row = {
            ...
            "mossy_mean_rate": float(m_mean.max()), #float(m_mean.mean()),
            "mossy_quantile_rate": mossy_quantile_value,
            "mossy_sem_rate": float(m_sem.mean()),
            ...
            "cosine_similarity_mossy_purk": float(cosine_similarity(m_mean, p_mean)),
            "cosine_similarity_stim_mossy": float(stim_to_mossy_cosine),
            ...
            "te_mossy_to_purk": compute_transfer_entropy_binned_old(m_mean, p_mean),
            "te_purk_to_mossy": compute_transfer_entropy_binned_old(p_mean, m_mean),
        ...
        ax1.fill_between(
            ...
            label=f"{int(q_low*100)}–{int(q_high*100)}% quantile range"
        )
        if stim is not None:
            ax1.plot(t, stim, color="red", alpha=0.1, linestyle='--', linewidth=0.75, label="Stimulus profile")
        ...
        ax2.set_title(f"Purkinje firing rate – Atrophy {atrophy:.1f}%")
        ax2.set_xlabel("Time (ms)")
        ax2.set_ylabel("Firing Rate (Hz)")
        ax2.legend()
        ax2.grid(True)
        
        # ➕ Imposta xlim per rimuovere primi/ultimi 500ms
        total_time = len(m_mean) * dt
        ax1.set_xlim(500, total_time - 500)
        ...
        """
        #################################################################################################
        plot_tau_fits_mean_rise_decay(m_mean, burst_onsets_ms, dt=0.1,
                              title="Mossy - Tau Fit Examples",
                              savepath=str(Path(base_dir) / f"tau_fit_mossy_k={k:.3f}.png"), color='green')
        
        plot_tau_fits_mean_rise_decay(p_mean, burst_onsets_ms, dt=0.1,
                              title="Purkinje - Tau Fit Examples",
                              savepath=str(Path(base_dir) / f"tau_fit_purkinje_k={k:.3f}.png"), color='red')


        # plots
        plot_firing_rate_panels(
            m_mean=m_mean,
            p_mean=p_mean,
            mossy=mossy,
            purk=purk,
            ...
            atrophy=atrophy,
            stim_to_mossy_cosine=stim_to_mossy_cosine,
            stim_to_purk_cosine=stim_to_purk_cosine,
            k=k,
            base_dir=base_dir,
        )
        ...
        # PSTH Mossy
        average_psth(spike_dfs[k], pop="mossy", event_times=burst_onsets_ms,
                     plot_raster=True,
                     save_path=f"./{base_dir}/mossy_psth_{k}.png",
                     condition_name=f"k={k:.3f}",
                     df_row_with_stim=row,
                     dt=0.1)
        # PSTH Purkinje
        average_psth(spike_dfs[k], pop="purkinje", event_times=burst_onsets_ms,
                     plot_raster=True, 
                     save_path=f"./{base_dir}/purkinje_psth_{k}.png",
                     condition_name=f"k={k:.3f}",
                     df_row_with_stim=row,
                     dt=0.1)
        # Raster Mossy
        plot_classical_raster(spike_dfs[k], row, pop="mossy",
                              window=(-200, 200),
                              save_path=f"./{base_dir}/mossy_raster_classic_{k}.png",
                              event_times=burst_onsets_ms,
                              title=f"Raster aligned to burst – k={k:.3f}")
        # Raster Purkinje
        plot_classical_raster(spike_dfs[k], row, pop="purkinje",
                              window=(-200, 200),
                              save_path=f"./{base_dir}/purkinje_raster_classic_{k}.png",
                              event_times=burst_onsets_ms,
                              title=f"Raster aligned to burst – k={k:.3f}")
    ...
    ####################################
    plot_epoched_firing_rate(
        df=df_summary,
        trace_column="mossy_timeseries",
        pop_name="Mossy Fibres",
        stim_column="stim_profile",
        onset_column="burst_onsets_ms",
        group_column="atrophy_percent",
        cmap="Oranges_r",
        operator=np.max,
        dt=0.1,
        save_path=f"./{base_dir}/mossy_epoched.png"
    )
    plot_epoched_firing_rate(
        df=df_summary,
        trace_column="purkinje_timeseries",
        pop_name="Purkinje",
        stim_column=None,#"stim_profile",
        onset_column="burst_onsets_ms",
        group_column="atrophy_percent",
        cmap="Blues_r",
        operator=np.max,
        dt=0.1,
        save_path=f"./{base_dir}/purkinje_epoched.png"
    )
    #######################################
    plot_firing_rate_vs_k(df_summary, save_path=f"./{base_dir}/fr_vs_atrophy.png")
    #plot_cross_correlation(df_summary)
    plot_cross_correlation(df_summary, base_dir)

    plot_time_series(df_summary, pop="mossy", base_dir=base_dir)
    plot_time_series(df_summary, pop="purkinje", base_dir=base_dir)
    plot_tf(df_summary,base_dir=base_dir)
    plot_stimulus_profile_per_k(df_summary, base_dir)
    plot_cosine_similarity_vs_atrophy(df_summary, column_name="cosine_similarity_stim_mossy", save_path=f"./{base_dir}/cosine_stim_mossy.png")
    plot_cosine_similarity_vs_atrophy(df_summary, column_name="cosine_similarity_stim_purk", save_path=f"./{base_dir}/cosine_stim_purk.png")
    plot_cosine_similarity_vs_atrophy(df_summary, column_name="cosine_similarity_mossy_purk", save_path=f"./{base_dir}/cosine_similarity_mossy_purk.png")
    plot_stim_vs_mossy_rate(df_summary, save_path=f"./{base_dir}/stim_vs_mf_rate.png")

    # crea una cartella che chiami postAnalysis e ci mv tutti i png
    """
    import shutil
    # 🔁 Raccoglie e sposta tutti i PNG generati in una cartella postAnalysis
    post_analysis_dir = Path(base_dir) / f"postAnalysis_{sigma}"
    post_analysis_dir.mkdir(parents=True, exist_ok=True)
    df_summary.to_csv(post_analysis_dir / "summary.csv", index=False)
    print("🧾 Summary salvato come CSV.")
    import shutil
    # 🔁 Crea cartella postAnalysis temporanea
    post_analysis_dir = Path(base_dir) / f"postAnalysis_{sigma}"
    post_analysis_dir.mkdir(parents=True, exist_ok=True)
    # 💾 Salva il summary
    df_summary.to_csv(post_analysis_dir / "summary.csv", index=False)
    print("🧾 Summary salvato come CSV.")
    # 📁 Crea cartella finale di raccolta
    collective_dir = Path(base_dir) / "B_collective_results" / "2.functions"
    collective_dir.mkdir(parents=True, exist_ok=True)
    # 🚚 Sposta tutti i file da postAnalysis in B_collective_results/2.functions
    for file in post_analysis_dir.glob("*"):
        shutil.move(str(file), collective_dir / file.name)
    # 🧹 Rimuove la cartella postAnalysis vuota
    post_analysis_dir.rmdir()
    print(f"📦 Tutti i file spostati in: {collective_dir}")
    

    
    #for png_file in Path(base_dir).glob("*.png"):
    #    shutil.move(str(png_file), post_analysis_dir / png_file.name)
    
    print(f"📁 Tutte le figure PNG sono state spostate in: {post_analysis_dir}")
    from datetime import datetime
    with open(post_analysis_dir / "analysis_log.txt", "w") as log:
        log.write(f"Esecuzione: {datetime.now()}\n")
        log.write(f"Condizione: {condition_name}\n")
        log.write(f"Top quartile: {use_top_quartile}, Modalità: {selection_mode}\n")
        log.write(f"Numero reti analizzate: {len(df_summary)}\n")
    """
...
"""
import shutil
def move_png_to_collective_results(path: Path):
    dest_dir = path / "B_collective_results" / "2.functions"
...
def main():
    ...
    process_reconstruction_yamls(base_dir, stimName=args.stimName)
    df_summary, spike_dfs = loadData(base_dir, mainEventKey="burst", 
                                     sigma=args.sigma,
                                     stimName=args.stimName,
                                     condition_name="saccades", use_top_quartile=True, selection_mode="percentile", method='mean')
    ...
    # Alla fine del main()
    move_png_to_sigma_folder(Path(args.path),sigma=args.sigma)
